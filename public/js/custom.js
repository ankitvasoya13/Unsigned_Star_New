...
!(function($) {
  ...
  //     type: "GET",
  //     url: "searchArtist",
  //     data: { text: text, country: country, genres: genres, type: type },
  ...
  //   searchTopArtist();
  // });
  // $("#TopArtistCountry").change(searchTopArtist);
  // $("#TopArtistGenres").change(searchTopArtist);

  // function searchTopArtist() {
  ...
  //     type: "GET",
  //     url: "searchTopArtist",
  //     data: { text: text, country: country, genres: genres },
  //     success: function(data) {
  //       //alert(JSON.stringify(data))
  ...
  $(document).delegate(".ms_tranding_more_icon", "click", function(e) {
    // $(".ms_tranding_more_icon").on("click", function (e) {
    if (
      (e.preventDefault(),
      e.stopImmediatePropagation(),
    ...
    jQuery.ajax({
      ...
      success: function(data) {
        if (data["message"] == "ok") {
          ...
          $("#InvalidUserPassword").html(data["message"]);
        }
      },
    });
  });

  var forgotForm = jQuery("#forgotForm");
  forgotForm.submit(function(e) {
    ...
    jQuery.ajax({
      ...
      success: function(data) {
        $("#InvalidEmail").html(
          '<div class="alert alert-success" role="alert">We have e-mailed your password reset link! </div>'
        );
        $("#forgotEmail").attr("value", "");
      },
      error: function(data) {
        $("#InvalidEmail").html(
          '<div class="alert alert-danger" role="alert">Sorry! your email is not found! </div>'
        );
  ...
  $("#justAnotherInputBox").keypress(function(e) {
    if (e.which == 13) {
      $("form#search").submit();
  ...
  //   audio.src = src;
  // }
  // getDuration("uploads/tracks/Tere Naal - Darshan Raval.mp3", function(length) {
  //   console.log("I got length " + Math.trunc(length / 60) + ":" + Math.trunc(length % 60));
  ...
  messageForm.submit(function (e) {
    ...
    if (panel_member_id === '') {      
      alert('Please select panel member from list.');
      return false;
    }
    $.ajax({
      type: "POST",
