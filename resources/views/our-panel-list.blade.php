...
<div class="col-lg-3 col-md-6 col-sm-12 col-xs-12">
    <div class="blog_category_box_wrapper blog_box_wrapper2 float_left">
        <div class="blog_news_img_wrapper float_left">
            ...
            <a href="{{ url('/profile/'.$user->id.'/'.$user->first_name . '-' . $user->last_name) }}"><img src="{{ asset('/uploads/'.$user->profile_image) }}"
                    alt="img" class="w-100"></a>
            @else
            <a href="{{ url('/profile/'.$user->id.'/'.$user->first_name . '-' . $user->last_name) }}"><img src="{{ asset('images/ft3.jpg') }}" alt="img"
                    class="w-100"></a>
            @endif
        </div>
        <div class="lest_news_cont_wrapper float_left">
            <div class="blog_heaidng_top">
                <h3> <a href="{{ url('/profile/'.$user->id.'/'.$user->first_name . '-' . $user->last_name) }}">{{ $user->first_name.' '.$user->last_name }}</a></h3>
            </div>
...
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
    @if ($total > $limit+4)
    <button class="btn-load" onclick="searchPanel({{$limit}})">Show more results</button>
    @endif
</div>